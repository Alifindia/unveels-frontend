...
export const CustomBilateralShader = {
  ...
  void main(){
    ...
    // Dynamic blur parameters based on mask value
    ...
    // Dynamic kernel radius based on mask intensity
    int kernelRadius = int(ceil(3.0 * dynamicSigmaSpatial));
    kernelRadius = min(kernelRadius, 10);
    ...
    // Enhance warm tones gradually based on mask intensity
    vec3 warmEnhanced = bilateralColor;
    warmEnhanced.r = min(bilateralColor.r * (1.0 + 0.05 * mask), 1.0); // Proportional red boost
    warmEnhanced.g = min(bilateralColor.g * (1.0 + 0.02 * mask), 1.0); // Proportional green boost

    // Calculate final color with graduated transitions proportional to mask value
    float blendFactor = smoothingStrength * mask;
